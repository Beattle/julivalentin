   <div id="tabs<?php echo $module;?>" class="htabs tab-category">
       <?php $c_h = 0; foreach($categories_tabs as $category) { ; ?>
        <a href="#tabs<?php echo $module;?>-<?php echo $c_h; ?>"><?php echo $category['name'];?>
            <input type="hidden" class="tab_href" value="index.php?route=module/ac_cms_set/tabitem&bc_id=<?php echo $category['bc_id']; ?>&module=<?php echo $module; ?>&bs_id=<?php echo $bs_id;?>" />
        </a>
        <?php $c_h++;} ?>
   </div>
   <?php for($z=0; $z<$c_h; $z++) { ?>
